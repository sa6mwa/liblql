...
static void assign_hit_indexes(lql_selector *selector, size_t *next) {
  size_t i;
  if (selector == NULL) {
    return;
  }
  if (selector_is_predicate(selector)) {
    selector->hit_index = *next;
    ++*next;
    return;
  }
...
static size_t selector_refresh_derived_lengths(lql_selector *selector) {
  size_t i;
  size_t child_max;
  size_t max_in;
  ...
  for (i = 0u; i < selector->child_count; ++i) {
    child_max = selector_refresh_derived_lengths(&selector->children[i]);
    if (child_max > max_in) {
      max_in = child_max;
    }
  }
...
selector_refresh_predicate_features(lql_selector *selector) {
  ...
  case LQL_SELECTOR_KIND_NE:
    if (selector->value_is_temporal) {
      ...
      selector->observer_family = LQL_EVAL_FAMILY_TEMPORAL;
  ...
  case LQL_SELECTOR_KIND_RANGE:
    ...
    selector->observer_family = selector->range_is_temporal
                                    ? LQL_EVAL_FAMILY_TEMPORAL
                                    : LQL_EVAL_FAMILY_NUMERIC_RANGE;
...
static void selector_clear_predicates(lql_selector_parser *ctx,
  ...
  ctx->allocator->destroy(ctx->allocator, selector->predicate_depth_indexes);
  ctx->allocator->destroy(ctx->allocator, selector->predicate_depth_offsets);
  selector->predicates = NULL;
  selector->predicate_depth_indexes = NULL;
...
static int prepare_predicate_depth_order(lql_selector_parser *ctx,
  ...
  cursor = NULL;
  selector->predicate_depth_offsets = (size_t *)ctx->allocator->calloc(
      ctx->allocator, selector->predicate_max_segment_count + 2u,
      sizeof(size_t));
  selector->predicate_depth_indexes = (size_t *)ctx->allocator->calloc(
      ctx->allocator, selector->predicate_count,
      sizeof(selector->predicate_depth_indexes[0]));
  ...
  for (i = 0u; i < selector->predicate_count; ++i) {
    ...
    selector->predicate_depth_indexes[cursor[depth]++] = i;
  }
...
static int clone_selector_payload(lql_selector_parser *ctx, lql_selector *dst,
  ...
  *dst = *src;
  ...
  dst->predicates = NULL;
  dst->predicate_depth_indexes = NULL;
