...
static int parse_temporal_fast(const char *raw, lql_temporal *out,
                               int allow_date_only, int allow_naive_datetime) {
  const char *p;
  size_t len;
  int y;
  int mo;
...
static int parse_temporal_literal_with_options(const char *raw,
                                               lql_temporal *out,
                                               int allow_date_only,
                                               int allow_naive_datetime,
                                               int allow_outer_space) {
  const char *p;
  int y;
  int mo;
  int d;
  ...
  if (parse_temporal_fast(raw, out, allow_date_only, allow_naive_datetime)) {
    return 1;
  }
  if (!allow_outer_space && isspace((unsigned char)*raw)) {
    return 0;
  }
  ...
  if (!allow_outer_space && isspace((unsigned char)*p)) {
    return 0;
  }
  while (isspace((unsigned char)*p)) {
    ++p;
...
LQL_INTERNAL_SYMBOL int lql_parse_temporal_literal(const char *raw,
  ...
  return parse_temporal_literal_with_options(raw, out, 1, 1, 1);
}

LQL_INTERNAL_SYMBOL int lql_parse_temporal_rfc3339_literal(const char *raw,
                                                           lql_temporal *out) {
  if (raw == NULL || out == NULL) {
    return 0;
  }
  return parse_temporal_literal_with_options(raw, out, 0, 0, 0);
}
