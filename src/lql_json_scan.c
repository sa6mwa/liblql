...
lql_json_match_textual_value_terms(const lql_json_scan *scan,
  ...
  for (i = 0u; i < scan->flat_term_count; ++i) {
    ...
    term = &scan->flat_terms[i];
    segment = segments == NULL ? 0u : segments[i];
    if (!lql_json_term_value_at(term, segment)) {
      continue;
    }
    if (term->kind == LQL_JSON_FLAT_TERM_PREFIX ||
        term->kind == LQL_JSON_FLAT_TERM_CONTAINS ||
        term->kind == LQL_JSON_FLAT_TERM_ICONTAINS ||
        term->kind == LQL_JSON_FLAT_TERM_IPREFIX) {
      active |= bit;
...
static lql_status lql_json_matched_scalar_value(lql_json_scan *scan,
  ...
  if (value != '{' && value != '[') {
    ...
    scalar_terms = 0ul;
    textual_terms = value == 'n' ? 0ul
                                 : lql_json_match_textual_value_terms(
                                       scan, matches, scan->match_term_segment);
    {
      size_t i;
      for (i = 0u; i < scan->flat_term_count; ++i) {
        unsigned long bit;
    ...
    if (scalar_terms == 0ul && textual_terms == 0ul && range_terms == 0ul &&
        number_eq_terms == 0ul)
      return lql_json_value(scan);
    if (literal != NULL && range_terms == 0ul) {
      lql_json_match_start(scan, textual_terms, 0, 0u);
    ...
    lql_json_match_start(scan, scalar_terms | textual_terms, 0, 0u);
...
static lql_status lql_json_object(lql_json_scan *scan) {
  ...
  for (;;) {
    ...
    if (key_matches && value == '"') {
      ...
      textual_terms = value == 'n'
                          ? 0ul
                          : lql_json_match_textual_value_terms(
                                scan, key_matches, scan->key_term_segment);
