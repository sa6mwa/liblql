...
static int
file_request_has_multiple_output_sinks(const lql_file_filter_request *request) {
  unsigned int sinks;
  sinks = 0u;
...
static int open_output_path_checked(const char *path, FILE *input, FILE **out,
...
static lql_status filter_open_stream(lql *self,
  ...
  status = self->stream_apply_spooled(self, &stream, &local_result, error);
  if (result != NULL) {
    *result = local_result;
  }
  ...
  if (request->count_only) {
    ...
    if (request->output_writer != NULL) {
      status = request->output_writer(request->output_user, count_buffer,
                                      (size_t)count_len, error);
      if (status != LQL_STATUS_OK && error != NULL &&
          error->code == LQL_STATUS_OK) {
        lql_set_error(error, status, "count writer failed");
      }
      return status;
    }
...
lql_status lql_rewrite_file_inline_spooled_internal(
  ...
  status =
      filter_open_stream(self, request, source, tmp, effective_result, error);
