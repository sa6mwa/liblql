...
#define lql_stream_apply lql_stream_apply_spooled
...
static lql_status test_file_open(void *user, lql_string_view path,
  ...
  ++source->opens;
  *out_reader =
      source->fail_read_no_error
          ? test_file_fail_read_no_error
          : (source->oversize_read ? test_file_oversize_read : test_read);
...
static lql_status test_fail_range_write_no_error(void *user, size_t offset,
                                                 size_t len,
                                                 lql_stream_writer_fn writer,
                                                 void *writer_user,
                                                 lql_error *error) {
...
static int run_selection(lql *ctx, const char *expr, const char *input,
  ...
  if (lql_stream_apply(ctx, &request, &result, &error) != LQL_STATUS_OK ||
...
static int run_scalar_json_semantic_regressions(lql *ctx) {
  ...
  static const char numeric_container_input[] = "{\"a\":{\"b\":3}}\n"
                                                "{\"a\":{\"b\":null}}\n"
                                                "{\"a\":{\"b\":false}}\n"
                                                "{\"a\":[3]}\n";
...
static int run_array_depth_state_regression(lql *ctx) {
  ...
  request.selector = selector;
  lql_error_init(&error);
  status = lql_stream_apply(ctx, &request, &result, &error);
  ctx->selector_destroy(ctx, selector);
...
static int run_post_hit_validation(lql *ctx) {
  ...
  memset(&decisions, 0, sizeof(decisions));
  request.reader_user = &reader;
  if (lql_stream_apply(ctx, &request, &result, &error) !=
...
static int run_direct_reader_eof_contract(lql *ctx) {
  ...
  request.reader_user = &reader;
  request.selector = selector;
  if (ctx->stream_apply(ctx, &request, &result, &error) != LQL_STATUS_OK ||
      result.records_seen != 1u || result.records_matched != 1u ||
...
static int run_mapped_string_predicates(lql *ctx) {
  ...
  if (run_selection(ctx, "contains{f=/msg,v=true}",
                    "{\"msg\":\"true value\"}\n"
                    "{\"msg\":true}\n"
                    "{\"msg\":\"false\"}\n",
                    3u, 2u)) {
  ...
  if (run_selection(ctx, "icontains{f=/msg,v=null}",
                    "{\"msg\":\"NULL value\"}\n"
                    "{\"msg\":null}\n"
                    "{\"msg\":\"none\"}\n",
                    3u, 1u)) {
  ...
  if (run_selection(ctx, "prefix{f=/msg,v=1}",
                    "{\"msg\":\"123\"}\n"
                    "{\"msg\":123}\n"
                    "{\"msg\":\"x1\"}\n",
                    3u, 2u)) {
  ...
  if (run_selection(ctx, "iprefix{f=/msg,v=true}",
                    "{\"msg\":\"TRUE value\"}\n"
                    "{\"msg\":true}\n"
                    "{\"msg\":\"false\"}\n",
                    3u, 2u)) {
  ...
  if (run_selection(ctx, "contains{f=/msg,a=true|null|1}",
                    "{\"msg\":\"true value\"}\n"
                    "{\"msg\":\"null\"}\n"
                    "{\"msg\":123}\n"
                    "{\"msg\":\"false\"}\n",
  ...
  if (run_selection(ctx, "contains{f=/msg,v=\"\"}", input, 3u, 3u) ||
      ...
      run_selection(ctx, "contains{f=/a,v=\"\"}", input, 3u, 2u)) {
    return 111;
  }
...
static int run_selector_selected_record_case(lql *ctx,
...
static int run_recursive_wildcard_selector_regression(lql *ctx) {
  ...
  if (run_selector_selected_record_case(ctx, "contains{field=/...}",
                                        terminal_recursive_null_child_input))
    return 1;
  if (run_selector_selected_record_case(ctx, "/*/...=1", recursive_any_input))
    return 1;
  ...
  if (run_selector_selected_record_case(ctx, "/x/.../**/[]=1",
    ...
    return 1;
  if (run_selector_selected_record_case(
          ctx, "/.../0/*=\"\"", recursive_object_numeric_key_wildcard_input))
    return 1;
  if (run_selector_selected_record_case(
          ctx, "/.../0/*=\"\"", recursive_nested_numeric_key_wildcard_input))
    return 1;
  if (run_selection(ctx, "/a/.../[]/0=1", recursive_array_branch_state_input,
                    1u, 0u))
    return 1;
  if (run_selection(ctx, "/a/.../[]/0!=1", recursive_array_branch_state_input,
                    1u, 1u))
...
static int run_wildcard_mutation_review_regressions(lql *ctx) {
  ...
  static const char recursive_after_wildcard_set_input[] =
      "{\"b\":{\"b\":1}}\n";
  ...
  static const char *const missing_increment_array_wildcard[] = {"/b/0/[]=+1"};
  static const char *const missing_increment_object_wildcard[] = {"/b/0/*=+1"};
...
static int run_mutation_error_output_is_atomic(lql *ctx) {
  ...
  static const char *const missing_create_then_bad_increment[] = {"/a/b=1",
                                                                  "/a++"};
  static const char *const missing_bad_nested_then_bad_increment[] = {"/a/b++",
                                                                      "/a++"};
...
static int run_wide_direct_mutation_action_state(lql *ctx) {
  ...
  if (lql_stream_apply(ctx, &request, &result, &error) != LQL_STATUS_OK ||
      ...
      memcmp(writer.data, expected, writer.len) != 0) {
    fprintf(stderr,
            "wide mutation action state failed\nexpected: %sactual: %.*s\n",
...
static int run_mutation_output(lql *ctx) {
  ...
  static const char wildcard_remove_array_index_input[] =
      "{\"items\":[[1,2]]}\n";
  ...
  if (lql_stream_apply(ctx, &request, &result, &error) != LQL_STATUS_OK ||
      result.records_seen != 1u || result.records_matched != 1u ||
      writer.len !=
          sizeof(wildcard_increment_array_wildcard_child_output) - 1u ||
  ...
  if (run_mutation_case(ctx, wildcard_increment_deep_object, 1u,
      ...
      run_mutation_case(ctx, scalar_late_recursive_wildcard_set, 1u,
                        scalar_late_wildcard_input,
                        scalar_late_wildcard_output) ||
      run_mutation_case(ctx, terminal_ellipsis_set, 1u, terminal_ellipsis_input,
...
static int run_byte_limit(lql *ctx) {
  ...
  request.limits.max_records = 1u;
  if (lql_stream_apply(ctx, &request, &result, &error) != LQL_STATUS_STOP ||
      result.stopped_early) {
    ctx->selector_destroy(ctx, selector);
    return 1;
  }
...
static int run_true_stream_contract(lql *ctx) {
  ...
  projection = NULL;
  lql_error_init(&error);
  if (ctx->stream_apply == NULL || ctx->stream_apply_spooled == NULL) {
    return 1;
  }
  if (ctx->selector_parse(ctx, "/status=\"open\"", &selector, &error) !=
      LQL_STATUS_OK) {
  ...
  request.on_value = test_value_writer_failure;
  request.value_user = &error_sink;
  if (ctx->stream_apply(ctx, &request, &result, &error) !=
          LQL_STATUS_CALLBACK_ERROR ||
      error_sink.count != 1u ||
      error_sink.write_status != LQL_STATUS_CALLBACK_ERROR ||
      error_sink.error_code != LQL_STATUS_CALLBACK_ERROR ||
  ...
  request.output_mode = LQL_STREAM_OUTPUT_SELECTED_RECORD;
  request.matched_only = 1;
  if (ctx->stream_apply(ctx, &request, &result, &error) != LQL_STATUS_STOP ||
      result.stopped_early) {
    ctx->selector_destroy(ctx, selector);
    return 1;
  }
...
static int run_capabilities(lql *ctx) {
  ...
  if (!caps.selector_parse || !caps.selector_inspection ||
      ...
      !caps.mutation_parse || !caps.stream_apply ||
      !caps.stream_apply_spooled || !caps.filter_file_spooled ||
...
static int run_file_filter_callback_output(lql *ctx) {
  ...
  memset(&request, 0, sizeof(request));
  request.input_path = path;
  request.output_writer = test_fail_write_no_error;
  request.selector = selector;
  request.count_only = 1;
  if (ctx->filter_file_spooled(ctx, &request, &result, &error) !=
          LQL_STATUS_CALLBACK_ERROR ||
      error.code != LQL_STATUS_CALLBACK_ERROR ||
      strstr(error.message, "count writer failed") == NULL) {
    ctx->selector_destroy(ctx, selector);
    remove(path);
    return 1;
  }
...
static int run_file_backed_mutations(lql *ctx) {
  ...
  if (run_file_backed_mutation_case(
      ...
      run_file_backed_mutation_case(
          ...
          "{\"payload\":\"AAEC\"}\n") ||
      run_file_backed_mutation_case(ctx,
                                    "base64file:/payload=/definitely/not/here",
                                    LQL_STATUS_IO_ERROR, "") ||
      run_file_backed_mutation_case(
...
static int run_virtual_file_source(lql *ctx) {
  ...
  request.output_mode = LQL_STREAM_OUTPUT_MUTATION;
  request.matched_only = 1;
  if (ctx->stream_apply_spooled(ctx, &request, &result, &error) !=
          LQL_STATUS_OK ||
...
static int run_clock_contract(lql *ctx) {
  ...
  static const char *const time_z_expression[] = {
      "time:/at=2025-01-01T12:34:56Z"};
  static const char *const time_offset_expression[] = {
      "time:/at=2025-01-01T14:34:56+02:00"};
  static const char *const time_fraction_expression[] = {
      "time:/at=2025-01-01T12:34:56.123456789Z"};
  static const char *const time_no_zone_expression[] = {
      "time:/at=2025-01-01T12:34:56"};
  static const char *const time_fraction_no_zone_expression[] = {
      "time:/at=2025-01-01T12:34:56.123456789"};
  static const char *const time_date_only_expression[] = {
      "time:/at=2025-01-01"};
  static const char *const time_quoted_no_zone_expression[] = {
      "time:/at='2025-01-01T12:34:56'"};
  static const char *const time_comma_fraction_expression[] = {
      "time:/at=2025-01-01T12:34:56,123Z"};
  static const char *const time_offset_hour_24_expression[] = {
      "time:/at=2025-01-01T12:34:56+24:00"};
  static const char *const time_offset_minute_60_expression[] = {
      "time:/at=2025-01-01T12:34:56+23:60"};
  static const char *const *const invalid_time_expressions[] = {
      time_no_zone_expression,         time_fraction_no_zone_expression,
      time_date_only_expression,       time_quoted_no_zone_expression,
      time_comma_fraction_expression,  time_offset_hour_24_expression,
      time_offset_minute_60_expression};
  ...
  static const char mutation_offset_output[] =
  ...
  mutation = NULL;
  lql_error_init(&error);
  if (ctx->mutation_parse(ctx, time_z_expression, 1u, &mutation, &error) !=
          LQL_STATUS_OK ||
      mutation == NULL) {
    ctx->mutation_destroy(ctx, mutation);
    return 1;
  ...
  mutation = NULL;
  lql_error_init(&error);
  if (ctx->mutation_parse(ctx, time_offset_expression, 1u, &mutation, &error) !=
          LQL_STATUS_OK ||
      mutation == NULL) {
    ctx->mutation_destroy(ctx, mutation);
    return 1;
  ...
  mutation = NULL;
  lql_error_init(&error);
  if (ctx->mutation_parse(ctx, time_fraction_expression, 1u, &mutation,
                          &error) != LQL_STATUS_OK ||
      mutation == NULL) {
    ctx->mutation_destroy(ctx, mutation);
    return 1;
  ...
  if (ctx->stream_apply_spooled(ctx, &request, &result, &error) !=
          LQL_STATUS_OK ||
      writer.len != sizeof(mutation_offset_output) - 1u ||
      memcmp(writer.data, mutation_offset_output, writer.len) != 0) {
    ctx->mutation_destroy(ctx, mutation);
    return 1;
  }
  ctx->mutation_destroy(ctx, mutation);
  mutation = NULL;
  /*
   * Selector temporal literals intentionally keep Go-compatible naive UTC
   * datetimes, but time: mutation values are strict RFC3339/RFC3339Nano: they
   * require a timezone-bearing timestamp and deliberately reject Go time.Parse
   * leniencies such as comma fractions or out-of-range offset components.
   */
  for (invalid_index = 0u;
       invalid_index <
       sizeof(invalid_time_expressions) / sizeof(invalid_time_expressions[0]);
       ++invalid_index) {
    lql_error_init(&error);
    if (ctx->mutation_parse(ctx, invalid_time_expressions[invalid_index], 1u,
                            &mutation, &error) != LQL_STATUS_PARSE_ERROR ||
        mutation != NULL) {
      ctx->mutation_destroy(ctx, mutation);
      return 1;
    }
  ...
  selector = NULL;
  lql_error_init(&error);
  if (ctx->selector_parse(ctx, "date{f=/timestamp,gte=2025-01-01 T00:00:00Z}",
                          &selector, &error) != LQL_STATUS_PARSE_ERROR) {
    if (selector != NULL)
      ctx->selector_destroy(ctx, selector);
    return 1;
...
static int run_wide_plan_stream_contract(lql *ctx) {
  ...
  request.reader_user = &reader;
  request.selector = selector;
  status = ctx->stream_apply(ctx, &request, &result, &error);
