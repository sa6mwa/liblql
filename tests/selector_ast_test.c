...
static void test_string_terms(lql *ctx) {
  static const char icontains_json_null[] =
      "{\"icontains\":{\"field\":\"/msg\",\"value\":null}}";
  lql_selector *selector;
  lql_selector_node root;
  lql_error error;
  char *json;
  ...
  selector = NULL;
  if (parse_selector(ctx, "contains_unquoted_bool_needle",
                     "contains{field=/msg,value=true}", 0, &selector)) {
    if (root_kind(ctx, "contains_unquoted_bool_needle", selector,
                  LQL_SELECTOR_NODE_CONTAINS, &root)) {
      expect_string_term(ctx, "contains_unquoted_bool_needle", root, "/msg", 1,
                         "true", 0, 0u);
    }
    json = selector_json(ctx, "contains_unquoted_bool_needle", selector);
    if (json != NULL) {
      if (!contains_text(json, "\"value\":\"true\"")) {
        fail("contains_unquoted_bool_needle",
             "string predicate scalar-looking needle must serialize as text");
      }
      free(json);
    }
    ctx->selector_destroy(ctx, selector);
  ...
  selector = NULL;
  if (parse_selector(ctx, "prefix_unquoted_number_needle",
                     "prefix{field=/msg,value=1}", 0, &selector)) {
    if (root_kind(ctx, "prefix_unquoted_number_needle", selector,
                  LQL_SELECTOR_NODE_PREFIX, &root)) {
      expect_string_term(ctx, "prefix_unquoted_number_needle", root, "/msg", 1,
                         "1", 0, 0u);
    }
    ctx->selector_destroy(ctx, selector);
  }
  ...
  selector = NULL;
  if (expect_ok("icontains_json_null_needle",
                ctx->selector_parse_json(ctx, icontains_json_null,
                                         strlen(icontains_json_null), &selector,
                                         &error),
                &error)) {
    if (root_kind(ctx, "icontains_json_null_needle", selector,
                  LQL_SELECTOR_NODE_ICONTAINS, &root)) {
      expect_string_term(ctx, "icontains_json_null_needle", root, "/msg", 1,
                         "null", 0, 0u);
    }
    json = selector_json(ctx, "icontains_json_null_needle", selector);
    if (json != NULL) {
      if (!contains_text(json, "\"value\":\"null\"")) {
        fail("icontains_json_null_needle",
             "JSON scalar string-predicate needle must serialize as text");
      }
      free(json);
    }
    ctx->selector_destroy(ctx, selector);
  ...
  selector = NULL;
  if (parse_selector(ctx, "contains_any_scalar_text_needles",
                     "contains{field=/msg,any=true|null|1}", 0, &selector)) {
    if (root_kind(ctx, "contains_any_scalar_text_needles", selector,
                  LQL_SELECTOR_NODE_CONTAINS, &root) &&
        expect_string_term(ctx, "contains_any_scalar_text_needles", root, "/msg",
                           0, "", 0, 3u)) {
      expect_string_any(ctx, "contains_any_scalar_text_needles", root, 0u,
                        "true");
      expect_string_any(ctx, "contains_any_scalar_text_needles", root, 1u,
                        "null");
      expect_string_any(ctx, "contains_any_scalar_text_needles", root, 2u, "1");
    }
    ctx->selector_destroy(ctx, selector);
  }
