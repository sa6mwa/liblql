#!/bin/sh
# Reference-only benchmark matrix and fixture generator.
#
# This file was copied from the pre-reset harness to preserve its exact data
# sets, selector rows, operation modes, counter checks, CPU pinning, and Go/C
# run order. It invokes deleted C and Lua paths and must not be run as-is.
# The direct-execution rewrite replaces only those invocations with a new C
# runner over lql_stream_execute. See docs/liblql-direct-execution-spec.md.
set -eu
