...
- `contains`, `icontains`, `prefix`, and `iprefix` use textual needles. Their
  `value` and `any` entries are strings even when selector text or selector AST
  JSON supplies scalar-looking values such as `true`, `null`, or `1`;
- string predicates compare against JSON string values and the textual form of
  JSON boolean and number values. JSON `null` is not treated as text;
...
Equality/inequality `value` parsing and `in` `any` parsing must preserve typed
JSON scalar behavior for unquoted selector text. Selector JSON parsing follows
the JSON value type directly for those typed operators. This deliberately
rejects the pinned Go library's scalar-to-string equality and membership
behavior for numbers, booleans, and null.
