...
`lonejson v0.38.0` or newer is the JSON substrate. liblql obtains lonejson
...
As of lonejson `v0.38.0`, the path-value visitor used by liblql still enforces
...
validated, and serialized through lonejson `v0.38.0` mapping, `JSON_VALUE`,
...
As of lonejson `v0.38.0`, the installed public header confirms that
...
`v0.38.0`. It does not claim the Go implementation's narrower mixed framing
...
- lonejson `v0.38.0` binary archive acquisition from GitHub release assets
  exists;
...
- callback-source mutation stream coverage now includes mixed scalar/object
  ...
  `v0.38.0` exposes `AUTO`, `NDJSON`, `SINGLE_VALUE`, `ARRAY_ITEMS`, and
...
- lonejson `v0.38.0` exposes candidate `stream_offset`, `byte_size`, and
...
- callback-source matched payload queries and sparse source mutation/projection
  replay now use lonejson `v0.38.0`
  ...
  pass-through numbers use lonejson chunked number writer callbacks
  ...
  `v0.38.0` V2 candidate transform surface so selector observation and
  projection/mutation writing share one validated parse for dense retained
  candidates; liblql must not emulate that integration with full-candidate
  buffering outside lonejson, temp-file staging, selector/result caches, or a
  second JSON parser. The consumed and remaining surfaces are summarized in
...
- current local lifecycle confidence has passed `make test-all`,
  ...
  compiler, linker, strip, and otool are available. lonejson `v0.38.0` does
