...
| Darwin x86_64 release artifacts | lonejson `v0.39.0` GitHub release asset inventory has no `liblonejson-0.39.0-x86_64-apple-darwin.tar.gz`; liblql dependency policy requires GitHub release SDK archives | Not a current package target |
...
| Privacy and relocatability verification | `make package-verify`, `make release`, package privacy fixtures including ELF and Darwin Mach-O runtime metadata negatives | Proven locally |
