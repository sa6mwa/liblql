...
def main():
    ...
    cases = [
        ...
        ("eq full", ["eq{field=/status,value=open}"]),
        ("eq alias", ["eq{f=/status,v=open}"]),
        ("eq full numeric literal", ["eq{field=/progress,value=75}"]),
        ("eq full boolean literal", ["eq{field=/active,value=true}"]),
        ...
        ("range gt lte", ["range{field=/progress,gt=40,lte=75}"]),
        ("range datetime quoted", ['range{field=/timestamp,gte="2025-01-01T00:00:00Z"}']),
        ...
        ("in alias", ["in{f=/status,a=open|queued}"]),
        ...
        ("not contains full", ["not.contains{field=/msg,value=degraded}"]),
        (
            "and prefixed",
        ...
        (
            "or indexed nested and full",
            [
                "or.0.eq{field=/region,value=us},or.1.and.0.range{field=/progress,gte=50},or.1.and.0.exists{/metadata/etag}"
            ],
        ),
